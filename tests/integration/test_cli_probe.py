...
from tests.fixtures.jev import FAKE_MODEL_ID
...
PROBE_ANSWERS = {
    ...
    "probe_score": {"score": 1.0, "probabilities": [0.2, 0.6, 0.2], "confidence": 0.6},
    "probe_noul": {"noul": 0.01},
}
...
def test_probe_prints_shape_and_raw() -> None:
    respx.post(URL).respond(
        200, json={"model": FAKE_MODEL_ID, "answers": PROBE_ANSWERS, "elapsedMs": 50}
    )
    code, out = run_probe()
    assert code == 0
    report = json.loads(out)
    assert report["parse"] == "ok"
    assert report["returned_model"] == FAKE_MODEL_ID
    assert report["shape"]["answers_location"] == "answers"
    assert report["shape"]["timing_fields"] == ["elapsedMs"]
    assert report["raw_response"]["answers"]["probe_noul"] == {"noul": 0.01}
